...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (document.querySelector('#pc-datepicker-6')) {
    flatpickr('#pc-datepicker-6', { inline: true });
  }
  // ApexCharts donut sparklines (replaced peity)
  document.querySelectorAll('.donut').forEach(function (el) {
    var values = el.textContent.split(',').map(Number);
    var total = values.reduce(function (a, b) { return a + b; }, 0);
    el.textContent = '';
    var options = {
      series: values.map(function (v) { return Math.round((v / total) * 100); }),
      chart: { type: 'donut', height: 32, width: 32, sparkline: { enabled: true } },
      colors: ['#ff9900', '#fff4dd', '#ffd592'],
      stroke: { width: 0 },
